...
/**
  Generate an `<amp-reddit/>` element configured to load the provided reddit post

  @function ampReddit
  @param {HTMLQuoteElement} blockquote blockquote surrounding embedded post
  @return {HTMLUnknownElement} the generated `<amp-reddit/>` element
*/
function ampReddit(blockquote) {
  const AMP_REDDIT = makeElement('<amp-reddit layout="responsive"/>');
  if (!blockquote) {
    return AMP_REDDIT;
  }

  const POST_ANCHOR = blockquote.querySelector('a'); // first link

  AMP_REDDIT.dataset.src = POST_ANCHOR.href;
  AMP_REDDIT.dataset.embedtype = 'post';

  AMP_REDDIT.setAttribute('width', '300');
  AMP_REDDIT.setAttribute('height', '400');
